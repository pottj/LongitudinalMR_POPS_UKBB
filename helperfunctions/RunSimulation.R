runSimulation = function(n_samples, n_times, n_sim, 
                         outfiles_dir, outfiles_prefix, save_data_perSim, do_plotting, counter, coresToUse,
                         SNPs_NR, SNPs_EAF, SNPs_centering, SNPs_Correct_ASs, SNPs_classes, SNP_beta_cov, SNP_beta_mean, SNP_beta_sd, AS_factor,
                         X_mean_random, X_var_random, X_covar_random, X_error_mean, X_error_sd, X_Sim_AS, X_Sim_GxE,
                         set_time, set_growth, AssocModelX,   
                         Y_mean_random, Y_var_random, Y_alpha, Y_binary, Y_binary_location,
                         MR_filterBadSNPs, MR_filterBadSNPs_treshold, MR_doCorrection){
  # DEBUG
  registerDoParallel(coresToUse)
  ...
  SimTab = foreach(s = 1:n_sim)%dorng%{
    ...
    #' ***
    {
      ...
      CoVarMatrix_SNPs[1,2] = SNP_beta_cov[1]
      CoVarMatrix_SNPs[2,1] = SNP_beta_cov[1]
      CoVarMatrix_SNPs[1,3] = SNP_beta_cov[2]
      CoVarMatrix_SNPs[3,1] = SNP_beta_cov[2]
      CoVarMatrix_SNPs[2,3] = SNP_beta_cov[3]
      CoVarMatrix_SNPs[3,2] = SNP_beta_cov[3]
    ...
    #' ***
    {
      ...
      #' Get sex variable
      sex = rbinom(n = n_samples, size = 1, prob = 0.5)
      table(sex)
      sex = sex+1
      ...
      if(X_Sim_AS == "X12" & X_Sim_GxE ==T){
        Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + beta11*age + 0.1*age2 + 0.5*sex*beta01 +
                                    rnorm(n=n_samples*n_times, mean=X_error_mean, sd=sqrt(X_error_sd))))
      }else if(X_Sim_AS == "X123" & X_Sim_GxE ==T){
        Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + beta11*age + 0.1*age2 + 0.5*sex*beta01 + epsilon) )
      }else if(X_Sim_AS == "X13" & X_Sim_GxE ==T){
        Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + 0.1*age2 + 0.5*sex*beta01 + epsilon) )
      }else if(X_Sim_AS == "X12" & X_Sim_GxE ==F){
        Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + beta11*age + 0.1*age2 +
                                    rnorm(n=n_samples*n_times, mean=X_error_mean, sd=sqrt(X_error_sd))))
      }else if(X_Sim_AS == "X123" & X_Sim_GxE ==F){
        Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + beta11*age + 0.1*age2 + epsilon) )
      }else if(X_Sim_AS == "X13" & X_Sim_GxE ==F){
    ...
    #' ***
    {
      ...
      myTabY[,sex := myTabX_long[scan==1,sex]]
      
      myTabY[,Y1 := sex + rnorm(n=n_samples,mean = Y_mean_random,sd = Y_var_random)]
      myTabY[,Y2 := sex + Y_alpha[1] * AS1 + rnorm(n=n_samples,mean = Y_mean_random,sd = sd(AS1))]
      myTabY[,Y3 := sex + Y_alpha[2] * AS2 * age + 
               rnorm(n=n_samples,mean = Y_mean_random,sd = sd(AS2))]
      myTabY[,Y4 := sex + Y_alpha[3] * AS3 + rnorm(n=n_samples,mean = Y_mean_random,sd = sd(AS3))]
      myTabY[,Y5 := sex + Y_alpha[1] * AS1 + Y_alpha[2] * AS2 * age + 
               rnorm(n=n_samples,mean = Y_mean_random,sd = sd(AS2)*sd(AS1))]
      myTabY[,Y6 := sex + Y_alpha[1] * AS1 + Y_alpha[3] * AS3 + 
               rnorm(n=n_samples,mean = Y_mean_random,sd = sd(AS3)*sd(AS1))]
      myTabY[,Y7 := sex + Y_alpha[3] * AS3 + Y_alpha[2] * AS2 * age + 
               rnorm(n=n_samples,mean = Y_mean_random,sd = sd(AS2)*sd(AS3))]
      myTabY[,Y8 := sex + Y_alpha[1] * AS1 + Y_alpha[2] * AS2 * age + Y_alpha[3] * AS3 +
      ...
      if(Y_binary==T){
        ...
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y1),location = Y_binary_location))
        myTabY[,Y1_bin := y_dummy]
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y2),location = Y_binary_location))
        myTabY[,Y2_bin := y_dummy]
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y3),location = Y_binary_location))
        myTabY[,Y3_bin := y_dummy]
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y4),location = Y_binary_location))
        myTabY[,Y4_bin := y_dummy]
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y5),location = Y_binary_location))
        myTabY[,Y5_bin := y_dummy]
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y6),location = Y_binary_location))
        myTabY[,Y6_bin := y_dummy]
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y7),location = Y_binary_location))
        myTabY[,Y7_bin := y_dummy]
        y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y8),location = Y_binary_location))
        myTabY[,Y8_bin := y_dummy]
    ...
    #' ***
    {
      #' Get associations
      if(Y_binary==T){
        myVars = names(myTabY)[-c(1:3)]
        myVars = myVars[grepl("_bin",myVars)]
        myMethod = "glm"
      }else{
        myVars = names(myTabY)[-c(1:3)]
        myMethod = "linReg" 
      ...
      myTab_SNPAssocs_Y[,outcome := gsub("_bin","",outcome)]
    ...
    #' ***
    {
      ...
      MRTab[exposure=="var",beta_IVW2 := beta_IVW*AS_factor[3]]
      MRTab[exposure=="var",SE_IVW2 := SE_IVW*AS_factor[3]]
