...
dumTab1 = foreach(j=1:length(myExposures))%do%{
  ...
  dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
    ...
    mod2 = gamlss(myX ~ myG + pn_sex + an_heightZ + an_smokstat + time + myG:time + ga2 + 
                  ...
                  sigma.formula = ~ time + pn_sex + an_heightZ + an_smokstat, 
...
myAssocs_X_gamlssIA[,table(pval_mean<0.05,pval_slope<0.05,phenotype)]

save(myAssocs_X_gamlssIA,file=paste0("../results/02_SNPs_03_SENS_noVar.RData"))
