...
SimTab = foreach(s = 1:n_sim)%dorng%{
  ...
  #' ***
  {
    ...
    y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y5)))
    myTabY[,Y5_bin := y_dummy]
    y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y6)))
    myTabY[,Y6_bin := y_dummy]
    y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y7)))
    myTabY[,Y7_bin := y_dummy]
    y_dummy = rbinom(n = n_samples, size = 1, prob = plogis(scale(myTabY$Y8)))
