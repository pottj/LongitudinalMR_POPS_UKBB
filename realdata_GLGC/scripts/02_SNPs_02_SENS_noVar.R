#' ---
#' title: "Get association of SNPs on TC (SENS: no variability)"
...
#' # Introduction ####
#' ***
#' Here, I want to estimate the SNP effects in a **gamlssIA** model for TC. I will estimate only estimate the SNP effects on the mean and slope, **not on the variability**.
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
  ...
  mod2 = gamlss(TC ~ sex + myG*exposure_age + lipLowMed +  
                ...
                sigma.formula = ~ sex + exposure_age + lipLowMed, 
...
save(myAssocs_X,file=paste0("../results/02_SNPs_02_SENS_noVar.RData"))
