...
#' check outcomes
myTab[,table(pn_emcsall,pn_sex)]

#' # Get effects ####
#' ***
names(myTab)
myOutcomes=names(myTab)[c(18)]
...
dumTab1 = foreach(j=1:length(myOutcomes))%do%{
  ...
  dumTab2 = foreach(i = 1:length(mySNPs))%do%{
    ...
    mod1 = glm(myX ~ myG + pn_ga_wk + pn_sex + an_est_age + an_height + an_smokstat + 
...
myAsscos_X_glm = rbindlist(dumTab1)
myAsscos_X_glm

myAsscos_X_glm[model=="glm",table(pval_mean<0.05,phenotype)]

save(myAsscos_X_glm,file=paste0("../results/08_7_Associations_SNPs_outcome_glm_",tag,".RData"))
