...
#' # Introduction ####
#' ***
#' **Main Figures for real data**
#' 
#' Forest plots per exposure on each outcome (generate all, but choose only the EFW_Z on BW_R)
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
dumTab0 = foreach(k = 1:length(myOutcomes))%do%{
  ...
  dumTab1 = foreach(i = 1:length(myExposures))%do%{
    ...
    filename = paste0("../results/_figures/MF_ForestPlots_",myX,"_",myY,".png")
