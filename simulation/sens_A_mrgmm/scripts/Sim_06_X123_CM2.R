...
#' ## Helper function ####
{
  ...
  source("../../../helperfunctions/MVMR_jp_sim_gmm.R")
...
#' These parameters should be fix for all simulations!
{
  ...
  X_mean_random = c(5,0.5)
  X_var_random = c(1,0.1)
  X_covar_random = 0
  X_error_mean = 0
  X_error_sd = 1
  X_age2 = -0.002
...
SimTab = foreach(s = 1:n_sim)%dorng%{
  ...
  #' ***
  {
    ...
    for(i in 1:n_times){
      myTabX_long[scan==i,age := rnorm(n=n_samples,mean=X_meanAge_bl + X_scanSteps*i,sd=0.5)]
    }
    ...
    if(X_Sim_AS == "AS1_AS2"){
      Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + beta11*age + X_age2*age2 + 
                                  rnorm(n=n_samples*n_times, mean=X_error_mean, sd=sqrt(X_error_sd))))
    }else if(X_Sim_AS == "AS1_AS2_AS3"){
      Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + beta11*age + X_age2*age2 + epsilon) )
    }else if(X_Sim_AS == "AS1_AS3"){
      Xsim = with(myTabX_long, (beta00 + beta01 + beta10*age + X_age2*age2 + epsilon) )
    }
    
    myTabX_long[,A := Xsim]
    
    #' Check AS association
    summary(lm(A ~ age + age2 + beta01 + beta11:age, data=myTabX_long, subset = scan==1))
    summary(lmer(A ~ age + age2 + beta01 + beta11:age + (1|ID), data=myTabX_long))
    summary(gamlss(A ~ age + age2 + beta01 + beta11:age  + random(x = as.factor(ID)),
                   sigma.formula = ~ age + age2 + tau01,
  ...
  {
    #' Run analysis
    MR_analysis_A = MVMR_jp_sim_gmm(data_GX_long = myTab_SNPAssocs_A,
