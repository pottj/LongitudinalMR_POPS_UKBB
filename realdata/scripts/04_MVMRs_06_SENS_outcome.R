...
#' # Introduction ####
#' ***
#' Sensitivity: using GY interaction effect instead of marginal effect
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
#' ## Outcome
load("../results/03_SNPs_02_SENS_Assocs_outcome_TIA_240319.RData")
myAssocs_Y = myAssocs_Y[SNP %in% SNPList[goodSNP==T,SNP]]
myAssocs_Y = myAssocs_Y[population == "all",]

#' ## save as temporary files
save(myAssocs_X_long,myAssocs_Y, file = paste0("../temp/04_MVMRInput_SENS_Outcome_",tag,".RData"))
...
dumTab2 = foreach(j = 1:length(myExposures))%dorng%{
  ...
  myAssocs_X_long2[,dumID := "sens_outcome"]
...
save(MVMR_results,file = paste0("../results/04_MVMR_07_SENS_outcome_",tag,".RData"))
