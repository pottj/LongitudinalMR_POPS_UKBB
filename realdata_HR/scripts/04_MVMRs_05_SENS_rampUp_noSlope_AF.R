#' ---
#' title: "MVMR - SENS analysis - ramp up - no slope - AF"
...
#' ## Exposure
load("../results/02_SNPs_05_SENS_RampUp_noSlope.RData")

#' Check the GX associations
myAssocs_X[,table(pval_mean==0)]
unplausibleSNPs = myAssocs_X[pval_mean == 0,SNP]
myAssocs_X = myAssocs_X[!is.element(SNP,unplausibleSNPs),]

myAssocs_X[,table(pval_mean<1e-50)]
myAssocs_X[,table(pval_var<5e-8)]
myAssocs_X[,table(pval_mean<1e-50,pval_var<5e-8)]
...
#' ## Outcome
load("../results/01_Prep_07_AFsummaryStats.RData")
...
myAssocs_Y = myAssocs_Y[rsID %in% myAssocs_X_long$SNP,]
LDTab = LDTab[SNP1 %in% goodSNPs$rsID,]
LDTab = LDTab[SNP2 %in% goodSNPs$rsID,]
save(myAssocs_X_long,myAssocs_Y,LDTab, file = paste0("../temp/04_MVMRInput_SENS_05_rampUp_noSlope.RData"))
...
save(MVMR_results,file = paste0("../results/04_MVMR_05_SENS_rampUp_noSlope_AF.RData"))
