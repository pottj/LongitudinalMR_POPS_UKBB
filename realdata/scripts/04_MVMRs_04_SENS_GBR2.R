...
#' # Introduction ####
#' ***
#' Sensitivity: using GBR2 instead of all2
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
#' ## Exposure
load("../results/02_SNPs_03_SENS_Assocs_exposure_gamlssIA_GBR2_gaQuad_240318.RData")
...
save(myAssocs_X_long,myAssocs_Y, file = paste0("../temp/04_MVMRInput_SENS_GBR2_",tag,".RData"))
...
dumTab2 = foreach(j = 1:length(myExposures))%dorng%{
  ...
  myAssocs_X_long2[,dumID := "sens_GBR2"]
...
save(MVMR_results,file = paste0("../results/04_MVMR_04_SENS_GBR2_",tag,".RData"))
