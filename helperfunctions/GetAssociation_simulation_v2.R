...
#' @export
GetAssociation_v2 = function(data,method,genotypes,dep_var_name,time_var_name,growth_var,getIA=T,sigmaCovars="SNP_time"){
  # debug
  ...
  stopifnot(method %in% c("linReg","glm","linMixed","gamlss","gamlssIA"))
    ...
    modTab = foreach(j = 1:SNPs_NR)%do%{
        ...
        if(dim(dummy1)[1]==3){
          res = data.table(SNP = c(j,j,j),
                           ...
                           tval = dummy1[,3],
                           pval = dummy1[,4])
        }else if(dim(dummy1)[1]==2 & getIA==F){
          res = data.table(SNP = c(j,j),
                           exposure = c("mean","var"),
                           beta = dummy1[,1],
                           SE = dummy1[,2],
        ...
        }else if(dim(dummy1)[1]==2 & getIA==T){
          res = data.table(SNP = c(j,j),
                           exposure = c("mean","slope"),
                           beta = dummy1[,1],
                           SE = dummy1[,2],
