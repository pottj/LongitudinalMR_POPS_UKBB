...
#' # Introduction ####
#' ***
#' Sensitivity: using LMM instead of GAM
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
#' ## Exposure
load("../results/02_SNPs_02_SENS_Assocs_exposure_linMixed_all2_gaQuad_240318.RData")
myAssocs_X = copy(myAssocs_X_linearMixed)
...
save(myAssocs_X_long,myAssocs_Y, file = paste0("../temp/04_MVMRInput_SENS_LMM_",tag,".RData"))
...
dumTab2 = foreach(j = 1:length(myExposures))%dorng%{
  ...
  myAssocs_X_long2[,dumID := "sens_LMM"]
...
save(MVMR_results,file = paste0("../results/04_MVMR_02_SENS_LMM_",tag,".RData"))
