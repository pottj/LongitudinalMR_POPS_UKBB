...
corrplot(dumMat[1:12,], is.corr = FALSE,col.lim = c(0, 1),col = COL1('Reds'), 
         addCoef.col = 'grey50',method = 'color',tl.col = "black",tl.srt = 45)

corrplot(dumMat[13:24,], is.corr = FALSE,col.lim = c(0, 1),col = COL1('Reds'), 
         addCoef.col = 'grey50',method = 'color',tl.col = "black",tl.srt = 45)
...
#' 
dumTab3 = cbind(dumTab_X1[,c(1,2,3,8,13),with=F],dumTab_X2[,c(2,3,8,13),with=F],dumTab_X3[,c(2,3,8,13),with=F])
x = dim(dumTab3)[2]
dumMat3 = as.matrix(dumTab3[,2:x])

colnames(dumMat3) = paste(rep(c("X1","X2","X3"),each=4),colnames(dumMat3),sep=" - ")
...
data_hlines = data.frame(type = c("mean","mean","slope","var"),
                          mylines = c(0.3,1.2,0.3,1))
