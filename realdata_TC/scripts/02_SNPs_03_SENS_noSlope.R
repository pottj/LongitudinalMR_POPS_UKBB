...
dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
  ...
  mod2 = gamlss(TC ~ sex + exposure_age + myG + lipLowMed + 
  ...
  data3 = copy(data2)
  data3 = data3[sex == 1,]
  mod2M = gamlss(TC ~ exposure_age + myG + lipLowMed + 
                   PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + random(x = as.factor(ID)),   
                 sigma.formula = ~ myG + exposure_age + lipLowMed, 
                 data = na.omit(data3), family = "NO")
  
  data4 = copy(data2)
  data4 = data4[sex == 2,]
  mod2F = gamlss(TC ~ exposure_age + myG + lipLowMed + 
                   PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + random(x = as.factor(ID)),   
                 sigma.formula = ~ myG + exposure_age + lipLowMed, 
  ...
  res1 = data.table(SNP = rep(pvar2[i,ID],3),
                    ...
                    beta_var = c(dummy2[2,1],dummy2M[2,1],dummy2F[2,1]),
                    SE_var =   c(dummy2[2,2],dummy2M[2,2],dummy2F[2,2]),
                    tval_var = c(dummy2[2,3],dummy2M[2,3],dummy2F[2,3]),
                    pval_var = c(dummy2[2,4],dummy2M[2,4],dummy2F[2,4]))
  res1
}
myAssocs_X = rbindlist(dumTab2)
myAssocs_X

save(myAssocs_X,file=paste0("../results/02_SNPs_03_SENS_noSlope.RData"))
