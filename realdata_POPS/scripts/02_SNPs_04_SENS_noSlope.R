...
dumTab1 = foreach(j=1:length(myExposures))%do%{
  ...
  dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
    ...
    mod2 = gamlss(myX ~ myG + pn_sex + an_heightZ + an_smokstat + time + ga2 + 
  ...
  #save(myAssocs_X,file=paste0("../temp/02_SNPAssocs_04_",myExposure,".RData"))
  myAssocs_X
}
...
save(myAssocs_X_gamlssIA,file=paste0("../results/02_SNPs_04_SENS_noSlope.RData"))
