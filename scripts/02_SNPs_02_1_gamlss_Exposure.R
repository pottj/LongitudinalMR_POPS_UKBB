#' ---
#' title: "Get association of PGS SNPs on exposure (2)"
...
#' # Introduction ####
#' ***
#' Here, I want to estimate the SNP effects in a Generalized Additive Models for Location, Scale and Shape (gamlss) for all relevant exposures. 
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
dumTab1 = foreach(j=1:length(myExposures))%do%{
  ...
  #save(myAsscos_X,file=paste0("../temp/08_2_Associations_SNPs_exposure_gamlss_",myExposure,"_",tag,".RData"))
  myAsscos_X
}
myAsscos_X_gamlss = rbindlist(dumTab1)
myAsscos_X_gamlss

myAsscos_X_gamlss[model=="gamlss",table(pval_mean<0.05,pval_var<0.05,phenotype)]

save(myAsscos_X_gamlss,file=paste0("../results/08_2_Associations_SNPs_exposure_gamlss_",tag,".RData"))
