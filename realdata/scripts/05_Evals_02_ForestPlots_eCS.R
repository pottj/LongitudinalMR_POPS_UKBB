...
#' # Introduction ####
#' ***
#' **Forest Plots** for all exposures on  emergency cesarean section (including the sensitivity runs LMM, and GBR3), seperate for mean, slope and var
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
MVMR1 = MVMR1[outcome %in% c("pn_emcsall"),]
MVMR1[outcome == "pn_emcsall", outcome := "eCS"]
...
dumTab1 = foreach(i=1:length(myExposures))%do%{
  ...
  dumTab2 = foreach(j=1:length(myExposureTypes))%do%{
    ...
    myXlab = paste0(myExposures[i]," (",myExposureTypes[j],") on emergency Caesarean Section")
