...
#' ## Helper function ####
{
  ...
  source("../../../helperfunctions/GetAssociation_RIRS.R")
...
SimTab = foreach(s = 1:n_sim)%dorng%{
  ...
  #' ***
  {
    ...
    # summary(lmer(A ~ beta01 + age + beta11:age + (1+age|ID), data=myTabX_long))
    # t2 = gamlss(A ~ beta01 + age + beta11:age + age2 + re(random=~1+age|as.factor(ID)), 
    #            sigma.formula = ~ age +tau01 ,
  ...
  #' ***
  {
    ...
    myTab_SNPAssocs_A = GetAssociation_RS(myData = dumTab,
                                       method = "gamlssIARS",
                                       genotypes = G2,
                                       dep_var_name = "A",
                                       time_var_name = "age")
    
    myTab_SNPAssocs_A = GetAssociation_RS(myData = copy(myTabX_long), 
                                       method = AssocModelX, 
                                       genotypes = G2, 
                                       dep_var_name = "A",
                                       time_var_name = "age")
