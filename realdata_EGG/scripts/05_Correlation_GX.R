...
myTab[phenotype == "efwcomb", phenotype := "EFW_Raw"]
myTab[phenotype == "logefwcomb", phenotype := "EFW_Log"]
myTab[phenotype == "efwcombZv2", phenotype := "EFW_ZScore"]
...
myMainPheno = "EFW_Log"
myOtherPhenos = myPhenos[-2]

myTab4 = copy(myTab)[phenotype == myMainPheno,]
...
for(i in 1:length(myOtherPhenos)){
  #i=3
  myTab2 = copy(myTab)[phenotype == myOtherPhenos[i]]
...
myTab4 = dcast(myTab4, SNP ~ phenotype, value.var=c("beta_mean","beta_slope","beta_var"))
myMatrix4 = as.matrix(myTab4[,-1])
CorTab4 = cor(myMatrix4,use = "pairwise.complete.obs")
