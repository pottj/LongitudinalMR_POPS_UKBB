...
matched = match(MVMR_results$exposure,matchingTable$old)
MVMR_results[,exposure := matchingTable[matched,new]]
MVMR_2sample_CS = copy(MVMR_results)
MVMR_2sample_CS[outcome=="CS_Sakaue",ID:="2SMR - any CS (own birth)"]
MVMR_2sample_CS[outcome=="CS_UKB_emergency",ID:="2SMR - emergency CS (childbirth)"]
MVMR_2sample_CS[outcome=="CS_UKB_elective",ID:="2SMR - elective CS (childbirth)"]
MVMR_2sample_CS[,outcome:="eCS"]


MVMR = rbind(MVMR_1Sample,MVMR_2sample_BW,MVMR_2sample_CS)
MVMR[,.N,ID]
...
dumTab0 = foreach(k = 1:length(myOutcomes))%do%{
  ...
  dumTab1 = foreach(i = 1:length(myExposures))%do%{
    ...
    dumTab2 = foreach(j = 1:length(myExposure_Types))%do%{
      ...
      filename = paste0("../results/_figures/07_2SampleMR_04_ForestPlots_vs_1SMR/",myX,"_",myY,"_",myExposure_Type,".png")
      if(myOutcome == "eCS"){
