...
#' - only age of 40-70 (statin treatment in older people could be higher ...)
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
filename = paste0("../results/_figures/01_Trajectory/SENS_TC_scan.png")
png(filename = filename,width = 1800, height = 1000, res=200)
plot(ggplot1)
dev.off()

filename = paste0("../results/_figures/01_Trajectory/SENS_TC_age.png")
png(filename = filename,width = 1800, height = 1000, res=200)
plot(ggplot2)
dev.off()

plotData[sex==1, sex2 := "men"]
...
ggplot3 = ggplot(plotData, aes(x=age3, y=exposure_value, fill=sex2)) +
  geom_hline(yintercept = 5.17,linetype="dashed", linewidth=0.5)+
  geom_hline(yintercept = 6.18,linetype="dotted", linewidth=0.5)+
  ...
  labs(x="Age (grouped)",
       y="Total cholesterol (in mmol/l)", 
       fill="Sex",
       title = paste0("Boxplots of TC levels per age group")) +
  theme_classic() 
ggplot3

filename = paste0("../results/_figures/01_Trajectory/SENS_TC_boxplots.png")
png(filename = filename,width = 1800, height = 1000, res=200)
plot(ggplot3)
dev.off()
...
#' # Save data ####
#' ***
save(myTab6, myTab7, file = paste0(UKB_phenotypes_filtered,"/01_Prep_05_UKB_GP_TC_GLGC_sens.RData"))
